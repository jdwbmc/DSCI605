...
## Create stats for Unemployment
statunemp <-describe(Unemployrate) %>%
  select(mean, sd, min, max, kurtosis, range)
statunemp[] <- lapply(statunemp, format, scientific = FALSE)
statunemp <- as.data.frame(statunemp[])
##############Crimerate
...
## Create stats for Crime
statcrime <-describe(Crimerate$Crimerate) %>%
  select(mean, sd, min, max, kurtosis, range)
statcrime[] <- lapply(statcrime, format, scientific = FALSE)
statcrime <- as.data.frame(statcrime[])
